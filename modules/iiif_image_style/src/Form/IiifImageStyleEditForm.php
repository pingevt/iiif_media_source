 ...
 */
class IiifImageStyleEditForm extends IiifImageStyleFormBase {

  /**
   * The image effect manager service.
   *
   * @var \Drupal\iiif_image_style\IiifImageEffectManager
   */
   ...
   *   The image effect manager service.
   */
  public function __construct(EntityStorageInterface $image_style_storage, IiifImageEffectManager $image_effect_manager) {
  ...
  public function form(array $form, FormStateInterface $form_state): array {
    ...
    $form['#tree'] = TRUE;
    $form['#attached']['library'][] = 'iiif_image_style/admin';
    ...
    foreach ($this->entity->getEffects() as $effect) {
      ...
      $links = [];
      $is_configurable = $effect instanceof IiifConfigurableImageEffectInterface;
      if ($is_configurable) {
        $links['edit'] = [
          'title' => $this->t('Edit'),
          'url' => Url::fromRoute('iiif_image_style.effect_edit_form', [
            'iiif_image_style' => $this->entity->id(),
            'image_effect' => $key,
          ]),
        ];
      ...
      $links['delete'] = [
        'title' => $this->t('Delete'),
        'url' => Url::fromRoute('iiif_image_style.effect_delete', [
          'iiif_image_style' => $this->entity->id(),
          'image_effect' => $key,
        ]),
      ];
  ...
  public function effectSave($form, FormStateInterface $form_state) {
    ...
    if (is_subclass_of($effect['class'], '\Drupal\iiif_image_style\IiifConfigurableImageEffectInterface')) {
      $form_state->setRedirect(
        'iiif_image_style.effect_add_form',
        [
          'iiif_image_style' => $this->entity->id(),
